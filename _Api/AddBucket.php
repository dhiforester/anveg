                            ...
                            if(empty($PostData['maksimal'])){
                                ...
                                if($ValidasiBucket!=="Valid"){
                                    ...
                                    if(!empty($ValidasiNamaBucket)){
                                        ...
                                        if($Input){
                                            //Cari id_bucket
                                            $QryParam = mysqli_query($Conn,"SELECT * FROM bucket WHERE id_akses='$id_akses' AND id_api_key='$id_api_key' AND nama='$nama'")or die(mysqli_error($Conn));
                                            $DataParam = mysqli_fetch_array($QryParam);
                                            if(empty($DataParam['id_bucket'])){
                                                $id_bucket="0";
                                            }else{
                                                $id_bucket=$DataParam['id_bucket'];
                                            }
                                            
                                            $SimpanLog=AddLog($Conn,$id_akses,"$id_api_key","API","Tambah Bucket");
